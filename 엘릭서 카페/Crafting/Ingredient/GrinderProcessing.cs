...
//분쇄기 가공 스크립트
public class GrinderProcessing : MonoBehaviour
{
    [SerializeField] private GameObject panel; //분쇄기 가공 UI 패널 오브젝트
    public Ingredient[] ingredients;           //가공 할 수 있는 재료 목록 (마나석 분말, 요정섩탕 분말)
    ...
    void Update()
    {
        ...
        if (isPanelActive)
        {
            if (Input.GetKeyDown(KeyCode.C) && !isProcessingFlags[0])
            {
            ...
            else if (Input.GetKeyDown(KeyCode.V) && !isProcessingFlags[1])
            {
                if (playerMoney.money > 9)
                {
                    playerMoney.DecreaseMoney(10); //재료 가격 10원
